...
const Index = () => {
  ...
  // Auto-open quiz popup after splash screen finishes
  useEffect(() => {
    const timer = setTimeout(() => {
      setQuizOpen(true);
    }, 6000); // ~6s — after splash animation completes
  ...
  return (
    <div className="min-h-screen bg-background" ref={container}>
      ...
      <div className="wave-divider">
        <svg viewBox="0 0 1440 70" preserveAspectRatio="none">
          <path fill="#FFF8F4" d="M0,50 C480,10 960,60 1440,30 L1440,70 L0,70 Z" />
        </svg>
      </div>
      <BlogsSection />
      ...
      {/* Risk Quiz Popup Modal */}
      <RiskQuizModal isOpen={quizOpen} onClose={() => setQuizOpen(false)} />
    </div>
  );
};
